        ...
        if (f != '__pycache__') and (f != 'test') and (f != 'testing'):  # test and cache directory are filtered
            if os.path.isfile(os.path.join(path, f)):  # inspect file
                if (pathlib.Path(f).suffix == ".py") and (not f.startswith("_") or f.startswith("__")):
                    ...
                    modulepath = os.path.splitext(os.path.join(path, f))[0]  # file path  # 优化，容易出错
                    modulepath = modulepath[modulepath.find(r"site-packages") + 14:len(modulepath)]  # 优化
                    modulepath = modulepath.replace('\\', '.')  # 优化
                    ...
                    try:
                        ...
                        if docs:
                            ...
                            if arxiv_index != -1:
                                ...
                                pdfModule.append(pdf)   # 这段代码似乎没有用到(看看是从哪到哪没用到 删去)
                        if (len(pdfModule) > 0):
